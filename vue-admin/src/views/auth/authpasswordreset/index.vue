...
async function loadPageData() {
  const { data } = await withLoading(useAuthPasswordResetApi().PageAuthPasswordReset(pageParams))
  pageData.value = data
}
loadPageData()
...
async function handleDelete(id: string | string[]) {
  ...
  const { success } = await useAuthPasswordResetApi().DeleteAuthPasswordReset(idArray)
  if (success) {
    loadPageData()
  }
}
