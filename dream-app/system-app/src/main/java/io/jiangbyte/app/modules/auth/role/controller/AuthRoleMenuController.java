...
public class AuthRoleMenuController {
    ...
    public Result<?> page(@ParameterObject AuthRoleMenuPageParam authRoleMenuPageParam) {
        return Result.success(authRoleMenuService.page(authRoleMenuPageParam));
    }

    @Operation(summary = "添加角色菜单关联")
    @SaCheckPermission("/auth/role/menu/add")
    @PostMapping("/auth/role/menu/add")
    public Result<?> add(@RequestBody @Valid AuthRoleMenuAddParam authRoleMenuAddParam) {
        authRoleMenuService.add(authRoleMenuAddParam);
    ...
    public Result<?> edit(@RequestBody @Valid AuthRoleMenuEditParam authRoleMenuEditParam) {
        authRoleMenuService.edit(authRoleMenuEditParam);
        return Result.success();
    }
    ...
    public Result<?> delete(@RequestBody @Valid @NotEmpty(message = "集合不能为空") List<AuthRoleMenuIdParam> authRoleMenuIdParam) {
        authRoleMenuService.delete(authRoleMenuIdParam);
        return Result.success();
    }

    @Operation(summary = "获取角色菜单关联详情")
    @SaCheckPermission("/auth/role/menu/detail")
    @GetMapping("/auth/role/menu/detail")
    public Result<?> detail(@ParameterObject @Valid AuthRoleMenuIdParam authRoleMenuIdParam) {
        return Result.success(authRoleMenuService.detail(authRoleMenuIdParam));
    }
